        ...
        private Monitor(IntPtr monitor, IntPtr hdc)
        {
            ...
            try
            {
                ...
                for (uint id = 0; EnumDisplayDevices(null, id, ref display, 0); id++)
                {
                    ...
                    AdapterName = adapter;
                    FriendlyName = string.IsNullOrWhiteSpace(display.DeviceString) ? "S.Recorder.Screen.Name.Internal" :
                        display.DeviceString == "Generic PnP Monitor" ? "S.Recorder.Screen.Name.Generic" : display.DeviceString;
                    break;
                }
            }
            catch (Exception ex)
